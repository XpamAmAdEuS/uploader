...
type RequestPreSendHook = (
  cb: (data: PreSendData) => PreSendResponse | Promise<PreSendResponse>,
) => void;

type BatchStartHook = (
  cb: (Batch: any, UploaderCreateOptions: any) => PreSendResponse | Promise<PreSendResponse>,
) => void;

const useBatchAddListener: BatchCancellableEventHook = generateUploaderEventHook(
  UPLOADER_EVENTS.BATCH_ADD,
  false,
);
const useBatchStartListener: BatchStartHook = generateUploaderEventHook(
  UPLOADER_EVENTS.BATCH_START,
);
const useBatchFinishListener: BatchEventHook = generateUploaderEventHook(
  UPLOADER_EVENTS.BATCH_FINISH,
);
const useBatchCancelledListener: BatchEventHook = generateUploaderEventHook(
  UPLOADER_EVENTS.BATCH_CANCEL,
);
const useBatchErrorListener: BatchEventHook = generateUploaderEventHook(
  UPLOADER_EVENTS.BATCH_ERROR,
);
const useBatchFinalizeListener: BatchEventHook = generateUploaderEventHook(
  UPLOADER_EVENTS.BATCH_FINALIZE,
);
const useBatchAbortListener: BatchEventHook = generateUploaderEventHook(
...
const useItemStartListener: ItemCancellableEventHook = generateUploaderEventHook(
...
const useItemFinalizeListener: ItemEventHook = generateUploaderEventHook(
  UPLOADER_EVENTS.ITEM_FINALIZE,
);

const useItemProgressListener: ItemEventHookWithState =
  generateUploaderEventHookWithState<BatchItem>(
    UPLOADER_EVENTS.ITEM_PROGRESS,
    (item: BatchItem) => ({ ...item }),
  );

const useRequestPreSend: RequestPreSendHook = generateUploaderEventHook(
  UPLOADER_EVENTS.REQUEST_PRE_SEND,
  false,
);

const useAllAbortListener: (cb: () => void) => void = generateUploaderEventHook(
