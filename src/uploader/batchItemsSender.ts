import { isFunction, logger, throttle } from '../shared';
import addLife from '../life-events';
import defaultSend from '../sender';
import { PROGRESS_DELAY, SENDER_EVENTS } from '../consts';
import { DEFAULT_OPTIONS, DEFAULT_PARAM_NAME } from './defaults';
import type { Batch, BatchItem, ItemsSender, UploaderCreateOptions } from '../types';

const reportItemsProgress = (
  items: BatchItem[],
  completed: number,
  loaded: number,
  trigger: any,
  ...
  items.forEach((item: BatchItem) => {
    logger.debugLog(
      `uploady.uploader.processor: file: ${item.id} progress event: loaded(${loaded}) - completed(${completed})`,
    );
...
const onItemUploadProgress = (items: BatchItem[], batch: Batch, e: ProgressEvent, trigger: any) => {
...
const createBatchItemsSender = (): ItemsSender => {
  const send = (items: BatchItem[], batch: Batch, batchOptions: UploaderCreateOptions) => {
    ...
    const throttledProgress = throttle(
      (e: ProgressEvent) => onItemUploadProgress(items, batch, e, trigger),
      PROGRESS_DELAY,
      true,
    );
