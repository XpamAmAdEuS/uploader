import { hasWindow, isProduction } from '../shared';
import { unwrap, isProxiable, isProxy } from '../simple-state';
import { DEFAULT_OPTIONS } from './defaults';

import type { UploaderCreateOptions, Destination } from '../types';
...
const getMandatoryOptions = (options?: UploaderCreateOptions): UploaderCreateOptions => {
  return {
    ...
    ...options,
    destination:
      options && options.destination ? getMandatoryDestination(options.destination) : null,
  };
};
  ...
  if (!isProduction()) {
    if (level < 3 && isProxy(obj)) {
      ...
      result = Array.isArray(obj)
        ...
        : Object.keys(obj).reduce<{ [p: string]: any }>((res, key) => {
