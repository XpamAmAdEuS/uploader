...
const getDestinationFromInput = (input: HTMLInputElement): DestinationShape => {
  ...
  if (form) {
    ...
    logger.debugLog(
      `Uploady.useFileInput: using custom input's parent form url ${destination.url} and method ${
        destination.method || ''
      }`,
    );
  }
...
const useFileInput = (fileInputRef?: InputRef): InputRef | undefined => {
