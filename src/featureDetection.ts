import { logger, request, merge } from './shared';
import { SUCCESS_CODES, KNOWN_EXTENSIONS, FD_STORAGE_PREFIX } from './consts';

import type { TusState, RequestResult } from './types';
import { safeSessionStorage } from './safe-storages';
...
const optionsConverter = {
  [KNOWN_EXTENSIONS.CONCATENATION]: (extensions: string[], tusState: TusState) => {
    ...
    if (+parallel > 1 && !~extensions.indexOf(KNOWN_EXTENSIONS.CONCATENATION)) {
      logger.debugLog(
        `tusSender.featureDetection: disabling parallel uploads, ${KNOWN_EXTENSIONS.CONCATENATION} extension not available`,
      );
      tusState.updateState((state: any) => {
  ...
  [KNOWN_EXTENSIONS.CREATION_WITH_UPLOAD]: (extensions: string[], tusState: TusState) => {
    if (
      tusState.getState().options.sendDataOnCreate &&
      !~extensions.indexOf(KNOWN_EXTENSIONS.CREATION_WITH_UPLOAD)
    ) {
      logger.debugLog(
        `tusSender.featureDetection: disabling create with data, ${KNOWN_EXTENSIONS.CREATION_WITH_UPLOAD} extension not available`,
      );
      tusState.updateState((state: any) => {
  ...
  [KNOWN_EXTENSIONS.CREATION_DEFER_LENGTH]: (extensions: string[], tusState: TusState) => {
    if (
      tusState.getState().options.deferLength &&
      !~extensions.indexOf(KNOWN_EXTENSIONS.CREATION_DEFER_LENGTH)
    ) {
      logger.debugLog(
        `tusSender.featureDetection: disabling defer length, ${KNOWN_EXTENSIONS.CREATION_DEFER_LENGTH} extension not available`,
      );
      tusState.updateState((state: any) => {
...
const handleResponse = (
  ...
  pXhr
    ...
    .then((response: void | XMLHttpRequest) => {
...
export const requestFeaturesFromServer = (
  url: string,
  tusState: TusState,
): { abort: () => boolean; request: Promise<void> } => {
  const { options } = tusState.getState();
...
const handleStoredValue = (
  ...
  try {
    ...
    if (parsed?.extensions) {
      ...
      logger.debugLog(
        `tusSender.featureDetection: retrieved feature detection data from session storage`,
        parsed,
      );
    ...
    logger.debugLog(
      `tusSender.featureDetection: failed to load feature detection data from session storage`,
    );
  }
  ...
  return parsed ? (null as any) : requestFeaturesFromServer(url, tusState);
};
