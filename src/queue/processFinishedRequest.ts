...
import { UPLOADER_EVENTS, ITEM_FINALIZE_STATES, FILE_STATES } from '../consts';
import { cleanUpFinishedBatches, incrementBatchFinishedCounter } from './batchHelpers';
import { finalizeItem } from './itemHelpers';
import type { ProcessNextMethod, QueueState, UploadData, BatchItem } from '../types';
...
const getIsFinalized = (item: BatchItem) => !!~ITEM_FINALIZE_STATES.indexOf(item.state);

const processFinishedRequest = (
  queue: QueueState,
  finishedData: FinishData[],
