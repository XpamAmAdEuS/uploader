...
import type { QueueState, BatchItem } from '../types';
import { FILE_STATES } from '../consts';

const getIsItemInActiveRequest = (queue: QueueState, itemId: string): boolean => {
  return queue
    .getState()
    .activeIds // $FlowIssue - no flat
    .flat()
    .includes(itemId);
};

const getIsItemReady = (item: BatchItem) => item.state === FILE_STATES.ADDED;
...
export const findNextItemIndex = (queue: QueueState): [string, number] => {
  ...
  while (batchId && !nextItemId) {
    if (getIsBatchReady(queue, batchId)) {
      ...
      while (
        nextItemId &&
        (getIsItemInActiveRequest(queue, nextItemId) || !getIsItemReady(items[nextItemId]))
      ) {
        itemIndex += 1;
        nextItemId = itemQueue[batchId][itemIndex];
      }
  ...
  return nextItemId ? [batchId, itemIndex] : (null as any);
};

export const getNextIdGroup = (queue: QueueState): any[] => {
  const state = queue.getState(),
    itemQueue = state.itemQueue,
    [nextBatchId, nextItemIndex] = findNextItemIndex(queue) || [];

  // eslint-disable-next-line prefer-const
  let nextId: any = nextBatchId && ~nextItemIndex ? itemQueue[nextBatchId][nextItemIndex] : null,
    nextGroup: any;
...
const updateItemsAsActive = (queue: QueueState, ids: any) => {
...
const processNextWithBatch = (queue: any, ids: any) => {
  let newBatchP;
  ...
  if (isNewBatchStarting(queue, ids[0])) {
    newBatchP = loadNewBatchForItem(queue, ids[0])
      ...
      .catch((err) => {
        logger.debugLog(
          'uploader.processor: encountered error while preparing batch for request',
          err,
        );
...
const processNext = (queue: QueueState): Promise<void> | void => {
  ...
  if (ids) {
    ...
    if (!currentCount || (concurrent && currentCount < maxConcurrent)) {
      ...
      processPromise = processNextWithBatch(queue, ids).then((failedOrCancelled: boolean) => {
