import { logger, merge, scheduleIdleWork } from '../shared';
import { unwrap } from '../simple-state';
import { BATCH_STATES, FILE_STATES, UPLOADER_EVENTS } from '../consts';
...
const getBatchFromState = (state: QueueStateData, id: string): Batch => state.batches[id].batch;
...
const removeBatchItems = (queue: QueueState, batchId: string) => {
  ...
  batch.items.forEach(({ id }: BatchItem) => finalizeItem(queue, id, true));
};
...
const cancelBatchForItem = (queue: QueueState, itemId: string) => {
  if (getIsItemExists(queue, itemId)) {
    ...
    if (batchId) {
      ...
      logger.debugLog(
        `uploady.uploader.batchHelpers: cancel batch called for batch already removed (item id = ${itemId})`,
      );
    }
  }
};
...
const loadNewBatchForItem = (queue: QueueState, itemId: string): Promise<boolean> => {
  ...
  return prepareBatchStartItems(queue, batch).then(({ cancelled }: ItemsSendData) => {
      ...
      if (orgItemCount === finishedCounter) {
        ...
        queue.updateState((state) => {
          if (state.currentBatch === batchId) {
            state.currentBatch = null as any;
          }
        });
...
const preparePendingForUpload = (queue: QueueState, uploadOptions?: UploadOptions): void => {
  queue.updateState((state: any) => {
    ...
    Object.keys(state.batches).forEach((batchId: string) => {
      ...
      if (batch.state === BATCH_STATES.PENDING) {
        batch.items.forEach((item: BatchItem) => {
          (item as any).state = FILE_STATES.ADDED;
        });
...
const removePendingBatches = (queue: QueueState): void => {
  ...
  Object.keys(batches)
    .filter((batchId: string) => batches[batchId].batch.state === BATCH_STATES.PENDING)
...
const getIsBatchFinalized = (batch: Batch): boolean => BATCH_FINISHED_STATES.includes(batch.state);
