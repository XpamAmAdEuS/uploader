import { logger, hasWindow, scheduleIdleWork } from '../shared';
...
import { QueueStateData } from '../types';
...
const createUploaderQueue = (
  ...
  const { state, update } = createState<QueueStateData>({
    itemQueue: {},
    batchQueue: [],
    currentBatch: null as unknown as string,
    batches: {},
    items: {},
    activeIds: [],
    aborts: {},
  ...
  const updateState = (updater: (arg0: QueueStateData) => void) => {
    update(updater);
  };
  ...
  const uploadPendingBatches = (uploadOptions: UploadOptions | undefined) => {
  ...
  const handleBatchProgress = (batch: Batch) => {
    ...
    if (batchItems) {
      const [completed, loaded] = batchItems.reduce(
        (res, { id }) => {
  ...
  const runCancellable = (name: string, ...args: any[]) => {
    // @ts-ignore
  ...
  if (hasWindow() && logger.isDebugOn()) {
    (window as any)[`__rpldy_${uploaderId}_queue_state`] = queueState;
  }
