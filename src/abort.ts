...
const abortNonUploadingItem = (item: BatchItem, aborts: AbortsMap, finalizeItem: any) => {
...
const abortAll = (
  ...
  const itemIds: string[] = Object.values(queue).flat();
...
const abortBatch = (
  ...
  const threshold =
    batchOptions.fastAbortThreshold === 0
      ? 0
  ...
  logger.debugLog(
    `abort: doing abort-batch on: ${batch.id} (${isFastAbort ? 'fast' : 'normal'} abort)`,
  );
