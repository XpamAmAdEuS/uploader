...
import { BatchItem, ChunkedSender, InitData, OnProgress, TusState, UploadData } from '../types';
...
const doChunkedUploadForItem = (
  ...
  const item = items[0];
  logger.debugLog(
    `tusSender.handler: init request finished. sending item ${item.id} as chunked`,
    initData,
  );
...
const handleParallelizedChunkInit = (
  items: BatchItem[],
  tusState: TusState,
  initData: InitData,
...
const handleTusUpload = (
  ...
  initRequest.then((initData: InitData): any => {
    let request,
      resumeFailed = false;
    ...
    if (initData) {
      if (initData.isDone) {
        logger.debugLog(
          `tusSender.handler: resume found server has completed file for item: ${items[0].id}`,
          items[0],
        );
        ...
        request = doChunkedUploadForItem(
          items,
          url,
          sendOptions,
          onProgress,
    ...
    if (resumeFailed) {
      logger.debugLog(
        `tusSender.handler: resume init failed. Will try creating a new upload for item: ${items[0].id}`,
      );
