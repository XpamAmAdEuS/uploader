import { request, logger } from '../shared';
import { addLocationToResponse, getUploadMetadata } from './utils';
import { FILE_STATES, SUCCESS_CODES } from '../consts';
import type { BatchItem, UploadData, TusState, SendOptions } from '../types';

const handleFinalizeResponse = (pXhr: any, chunkedUploadData: UploadData): Promise<UploadData> =>
  pXhr
    ...
    .then((xhr: XMLHttpRequest) => {
      ...
      const resLocation =
        successCode && xhr && 'getResponseHeader' in xhr && xhr.getResponseHeader('Location');

      if (resLocation) {
        logger.debugLog(
          `tusSender.finalizeParallel: successfully finalized parallel upload`,
          resLocation,
        );
        ...
        result = {
          ...
          response: {
            message:
              xhr?.response ||
              (successCode && !resLocation ? 'No valid location header for finalize request' : ''),
          },
        };
      }
...
const finalizeParallelUpload = (
  ...
  chunkedRequest.then((chunkedUploadData: UploadData) => {
    let finalResult: any = chunkedUploadData;
    ...
    if (chunkedUploadData.state === FILE_STATES.FINISHED) {
      ...
      if (itemData) {
        ...
        if (parallelUploadUrls.length !== chunkItemIds.length) {
          throw new Error(
            `tusSender: something went wrong. found ${parallelUploadUrls.length} upload urls for ${chunkItemIds.length} chunks`,
          );
        }
        ...
        const pXhr: any = request(url, null as any, { method: 'POST', headers });
