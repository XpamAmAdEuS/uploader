...
import { CHUNK_EVENTS, FILE_STATES, SUCCESS_CODES, UPLOADER_EVENTS } from '../consts';
import {
  ChunkedSender,
...
const handleParallelChunk = (
  ...
  return initResult.request.then((response: UploadData) => {
    return response.state !== FILE_STATES.FINISHED;
  });
};

const updateChunkStartData = (tusState: TusState, data: ChunkStartEventData, isParallel: any) => {
...
const handleEvents = (
  ...
  if (CHUNKING_SUPPORT) {
    uploader.on(UPLOADER_EVENTS.ITEM_FINALIZE, (item) => {
      ...
      if (itemData) {
        logger.debugLog(
          `tusSender.handleEvents: item ${item.id} finalized (${item.state}). Removing from state`,
        );
        ...
        if (options.forgetOnSuccess) {
          logger.debugLog(
            `tusSender.handleEvents: forgetOnSuccess enabled, removing item url from storage: ${item.id}`,
          );
    ...
    uploader.on(CHUNK_EVENTS.CHUNK_FINISH, ({ item, chunk, uploadData }: ChunkFinishEventData) => {
      ...
      if (isParallel) {
        if (options.forgetOnSuccess) {
          ...
          logger.debugLog(
            `tusSender.handleEvents: forgetOnSuccess enabled, removing parallel chunk url from storage: ${parallelId}`,
          );
        ...
        logger.debugLog(
          `tusSender.handleEvents: received upload response (code: ${status}) for : ${item.id}, chunk: ${chunk.id}`,
          response,
        );
